...
3. Poll the appropriate queue: `GET /api/v1/agent/applications/company-research-pending`, `.../ppa-pending`, or `.../application-pending` (FIFO, configurable limit). Use `POST /api/v1/agent/workers/assign` / `release` to track concurrent workers per stage.
...
5. Enrich companies via `PUT /api/v1/agent/companies/{id}` or `PATCH /api/v1/agent/companies/bulk`; queue companies needing research via `GET /api/v1/agent/companies/unindexed` (`research_status=pending`).
